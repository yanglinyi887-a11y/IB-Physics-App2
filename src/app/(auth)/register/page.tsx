...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signInRes?.error) { setError("Account created but sign-in failed. Please try logging in."); setLoading(false) }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 bg-zinc-950">
      <Card className="w-full max-w-sm border-zinc-800 bg-zinc-900">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="relative"><Separator className="absolute top-3" /><span className="relative flex justify-center"><span className="bg-zinc-900 px-2 text-xs text-zinc-500 uppercase">or</span></span></div>
